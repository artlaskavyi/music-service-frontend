(function() {
    ...
    function renderSearchResults(results, container, query) {
        ...
        if (results.artists && results.artists.length > 0) {
            foundResults = true;
            html += '<h3>Artists</h3><div class="card-grid">';
        ...
        if (results.albums && results.albums.length > 0) {
            foundResults = true;
            html += '<h3>Albums</h3><div class="card-grid">';
        ...
        if (results.songs && results.songs.length > 0) {
            foundResults = true;
            html += '<h3>Songs</h3><ul class="search-result-list">';
            results.songs.forEach(song => {
                html += `<li class="search-result-item">
                            <span class="track-title">${song.title}</span>
                            ${song.artist ? `<span class="artist-name">by ${song.artist.artistName}</span>` : ''}
                            ${song.album ? `<span class="album-title">from ${song.album.title}</span>` : ''}
        ...
        if (results.playlists && results.playlists.length > 0) {
            foundResults = true;
            html += '<h3>Playlists</h3><ul class="search-result-list">';
            results.playlists.forEach(playlist => {
                html += `<li class="search-result-item">
                            <span class="playlist-title">${playlist.name}</span>
                            ${playlist.user ? `<span class="user-owner">by ${playlist.user.username}</span>` : ''}
                         </li>`;
            });
            html += '</ul>';
        }
        ...
        if (results.users) {
            const usersData = Array.isArray(results.users) ? results.users : (results.users ? [results.users] : []);
            if (usersData.length > 0) {
                foundResults = true;
                html += '<h3>Users</h3><ul class="search-result-list">';
